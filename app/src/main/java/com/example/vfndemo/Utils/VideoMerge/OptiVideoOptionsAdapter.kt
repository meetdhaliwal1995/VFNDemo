...
package com.example.vfndemo.Utils.VideoMerge
...
import com.example.vfndemo.Utils.MergeVideo.OptiConstant
...
class OptiVideoOptionsAdapter(videoOptions: ArrayList<String>, val context: Context, optiVideoOptionListener: OptiVideoOptionListener, orientationLand: Boolean) :
    ...
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): MyPostViewHolder {
        return if(myOrientationLand) {
            MyPostViewHolder(LayoutInflater.from(context).inflate(R.layout.video_opti_option_view_land, p0, false))
        } else {
            MyPostViewHolder(LayoutInflater.from(context).inflate(R.layout.video_opti_option_view, p0, false))
        }
    }
...
//            }
            ...
            OptiConstant.MERGE -> {
                p0.ivOption.setImageResource(R.drawable.ic_play)
            }
